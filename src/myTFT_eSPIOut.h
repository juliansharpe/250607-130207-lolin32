	    ...
	    public:
          ...
          return memWord(&(stat==enabledStatus?colors[color].enabled[selected+edit]:colors[color].disabled[selected]));
		    ...
		    void setCursor(idx_t x,idx_t y,idx_t panelNr=0) override {
					const panel p=panels[panelNr];
					gfx.setCursor((p.x+x)*resX+6,(p.y+y)*resY+fontMarginY+1);
				}
				...
				void drawCursor(idx_t ln,bool selected,status stat,bool edit=false,idx_t panelNr=0) override {
					...
					//gfx.drawRect(p.x*resX+2,(p.y+ln)*resY+2,16,16,getColor(cursorColor,selected,enabledStatus,false));
                    gfx.drawRoundRect(
                        p.x*resX, (p.y+ln)*resY,
                        maxX()*resX, resY,
                        3, getColor(cursorColor, selected, enabledStatus, false)
                    );
				}
	  };
